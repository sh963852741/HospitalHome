<template>
    <i-card id="review-hospital-list">
        <i-row class="searcher" type="flex">
            ...
            <i-col span="6">
                <i-input prefix="ios-search" size="large" placeholder="搜索部门名称或医院名称" v-model="keyword" @keyup.enter.native="getData" />
            </i-col>
        </i-row>
...
export default {
    methods: {
        goTo (id) {
            ...
            this.$router.push({ name: `Depart${page}Detail`, query: {id} });
        },
        getData (page, filter) {
            let params = {
                page: page || this.page,
            ...
            axios.post("/api/hospital/GetDeparts", params, msg => {
                if (msg.success) {
                    this.data = msg.data;
