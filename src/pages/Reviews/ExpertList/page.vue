<template>
    <i-card id="review-hospital-list">
        <i-row class="searcher" type="flex">
            ...
            <i-col span="6">
                <i-input prefix="ios-search" size="large" placeholder="搜索专家名称或医院名称" v-model="keyword" @keyup.enter.native="getData" />
            </i-col>
        </i-row>
        ...
        <i-table class="data-table" stripe :columns="columns" :data="data" ref="dataTable">
            ...
            <template slot-scope="{row}" slot="belong">
                {{ row.departName }} / {{ row.hospitalName }}
            </template>
...
export default {
    methods: {
        goTo (id) {
            ...
            this.$router.push({ name: `Expert${page}Detail`, query: {id} });
        },
        getData (page, filter) {
            let params = {
                page: page || this.page,
            ...
            axios.post("/api/hospital/GetExperts", params, msg => {
                if (msg.success) {
                    this.data = msg.data;
                    ...
                    if (filter) {
                        ...
                        dt.handleFilterSelect(3, filter);
                    }
                }
            })
        },
    ...
    data () {
        let dic = app.dictionary["ApproveStatus"];
        let ApproveStates = [];
